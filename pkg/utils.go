...
func WalkDirs(curr string, fileIngredientsPtr *[]FileIngredients, processedSteps []string, ignoreList []string) bool {
	...
	check(err, true)
	rootDirPathList := strings.Split(rootDirPath, "/")
	rootDir := rootDirPathList[len(rootDirPathList)-1]

	currDirPath := strings.Split(curr, "/")
	currDirName := currDirPath[len(currDirPath)-1]
